...
GetPIJFromDir_Rnd <- function(ConDir){
	ConFiles   = list.files(path=ConDir,pattern=Type,full.names=T,recursive=T)
	# ConFiles   = ConFiles[1:100]
	ConWeight  = GetWeight(ConFiles)
	
	MajorInd   = which(ConWeight * NumMax >= 1)
	cat (paste("MajorInd = ",length(MajorInd)),"\n")

	ConFiles   = ConFiles[MajorInd]
	ConWeight  = ConWeight[MajorInd]
...
RndConFiles=list.files(path=RndConDir,pattern=Type,full.names=T,recursive=T)
...
NumMax = 100
RndConWeightPIJList = GetPIJFromDir_Rnd(RndConDir)
